  ...
  describe 'basic destruction' do
    ...
    context 'a node without relationships' do
      it 'quits out of the process without performing an expensive match' do
        expect_any_instance_of(ActiveGraph::ActiveNode::Query::QueryProxy).not_to receive(:unique_nodes_query)
        node.destroy
      end
    end
    ...
    context 'a node with relationships' do
      ...
      it 'continues as normal' do
        expect_any_instance_of(ActiveGraph::ActiveNode::Query::QueryProxy).to receive(:unique_nodes_query).and_call_original
        node.destroy
      end
    end
    ...
    context 'delete relationships' do
      context 'dependent destroy_orphans' do
        ...
        it 'deletes only orphans' do
          ...
          expect { Band.find(orphan_band.id) }.to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
          expect { Band.find(non_orphan_band.id) }.not_to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
        end
      end
      ...
      context 'dependent destroy' do
        ...
        it 'cascades dependent destroy' do
          tour.routes = [route_2]
          expect { Route.find(route.id) }.to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
          expect { Comment.find(comment.id) }.to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
        end
      end
      ...
      context 'dependent delete' do
        ...
        it 'deletes dependent model node without cascade' do
          ...
          expect { BadModel.find(bad_model.id) }.to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
          expect { Stop.find(stop_2.id) }.not_to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
        end
      end
      ...
      context 'dependent delete_orphans' do
        ...
        it 'deletes only orphans' do
          ...
          expect { Comment.find(comment_1.id) }.to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
          expect { Comment.find(comment_2.id) }.not_to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
        end
      end
    end
    ...
    context 'destroy ActiveRel relationship' do
      context 'dependent destroy' do
        ...
        it 'destroys the BadModel and Stop on deletion of ActiveRel' do
          rel.destroy
          expect { Stop.find(city.id) }.to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
          expect { BadModel.find(bad_model.id) }.to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
        end
      end
      ...
      context 'dependent destroy orphans' do
        ...
        it 'deletes only orphans' do
          rel_1.destroy
          expect { BadModel.find(bad_model_1.id) }.to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
           expect { BadModel.find(bad_model_2.id) }.not_to raise_error(ActiveGraph::ActiveNode::Labels::RecordNotFound)
        end
      end
