...
describe "Versioning" do
  ...
  it "should create correctly named incoming and outgoing relationships to and from snapshots" do
    ...
    ferarri.incoming(:sports_cars).count.should == 1 #Versioning uses a relationship name with a prefix
  ...
  it "should not version rule and version to snapshot relationships" do
    ...
    model1.save!
    model1.version(1).rels.count().should == 1
    model1.version(1).rels.first.rel_type.should == :version
    model1.version(2).rels.count().should == 1
    model1.version(2).rels.first.rel_type.should == :version
  end
  ...
  it "restores an older version with relationships" do
    ...
    driver.save!
    driver.current_version.should == 3
    driver._java_node.rels.count.should == 6 #1 to the Driver _all node, 3 snapshots, 2 sports cars
    driver.revert_to(1)
    driver.sports_cars.should be_empty
    pending "Does not work for some reason"
    driver.current_version.should == 4
    driver._java_node.rels.count.should == 5 #4 relationships to snapshots + 1 to the Driver _all node
    driver.revert_to(2)
