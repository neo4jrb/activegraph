...
describe Neo4j::Functions::Count, :type => :transactional do
  ...
  context "rule(:young, :functions => Count.new){ age < 30}" do
    before(:all) do
      ...
      @clazz.rule(:young, :functions => Neo4j::Functions::Count.new) { age && age < 30 }
    end
...
describe Neo4j::Functions::Sum, :type => :transactional do
  ...
  context "rule :all, :functions => Sum.new(:age)" do
    before(:all) do
      @clazz.rule :all, :functions => Neo4j::Functions::Sum.new(:age)
    end
  ...
  context "rule :old, :functions => Sum.new(:age)" do
    before(:all) do
      @clazz.rule(:old, :functions => Neo4j::Functions::Sum.new(:age)) { age && age > 20 }
    end
