...
describe "Neo4j::RailsNode Relationships", :type => :integration do
  ...
  context "traversal" do
    ...
    it "should still support outgoing traversals of depth > 1" do
      @n1.outgoing(:foo).depth(2).count.should == 2
      @n1.outgoing(:foo).depth(2).should include(@n2, @n3)
    end

    it "should still support incoming traversals of depth > 1" do
      @n3.incoming(:foo).depth(2).count.should == 2
      @n3.incoming(:foo).depth(2).should include(@n2, @n1)
    end

    it "should still support incoming traversals of depth == 1" do
      @n3.incoming(:foo).depth(1).count.should == 1
      @n3.incoming(:foo).depth(1).should include(@n2)
    end

    it "should support prune traversal method" do
      @n3.outgoing(:foo).prune{|path| path.end_node[:name] == 'B'}.count.should == 0
    end
  ...
  context "has_one" do
    ...
    describe "node.favorite.delete" do
      it "should delete relationship and node" do
        @movie_class.find(@movie_1.id).should_not be_nil
        @actor.favorite.delete
        @actor.favorite.should == nil
        @movie_class.find(@movie_1.id).should be_nil
        @actor.favorite_rel.should == nil
      end

    end

    describe "node.favorite_rel.delete" do
      it "should only delete the relationship" do
        @movie_class.find(@movie_1.id).should_not be_nil
        @actor.favorite_rel.delete
        @actor.favorite.should == nil
        @movie_class.find(@movie_1.id).should_not be_nil
        @actor.favorite_rel.should == nil
      end
  ...
  context "has_n" do
    ...
    describe "build outgoing on rel" do
      before(:each) do
        @ret = @actor.acted_in_rels.build(:since => 1994)
      end

      it "should allow to build a relationship with no properties" do
        rel = @actor.acted_in_rels.build
        rel.props.count.should == 1
      end

      it "create a new node but does not save it" do
        @actor.acted_in.count.should == 4
      end

      it "create a new node but does not save it" do
        @actor.acted_in_rels.find { |x| x[:since] == 1994 }.should_not be_persisted
      end

      it "does not create a new relationship" do
        @actor.reload
        @actor.acted_in.count.should == 3
      end
    ...
    describe "create outgoing on rel" do
      before(:each) do
        @ret = @actor.acted_in_rels.create(:since => 1994)
      end

      it "create a new node and save it" do
        @actor.acted_in_rels.find { |x| x[:since] == 1994 }.should be_persisted
        @actor.acted_in_rels.count.should == 4
        @actor.acted_in.count.should == 4
      end
    ...
    describe "delete_all on outgoing relationships" do
      it "deletes all relationship" do
        ...
        @actor.acted_in.count.should == 0
        @actor.acted_in_rels.count.should == 0
    ...
    describe "delete_all on incoming nodes" do
      it "deletes all relationship" do
        ...
        @movie_2.actors_rels.count.should == 0
    ...
    describe "delete on incoming rels" do
      it "can delete incoming" do
        @movie_2.actors.count.should == 2
        @movie_2.actors.delete(@actor)
        @movie_2.actors.count.should == 1
    ...
    describe "delete on outgoing rels" do
      ...
      it "does not destroy the objects" do
        ModelRelationship1.all.count.should == 3
        @actor.acted_in.delete(@movie_2, @movie_1, @movie_3)
        ModelRelationship1.all.count.should == 0
