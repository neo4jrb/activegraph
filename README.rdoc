...
This has been verified to work on JRuby 1.1.5 and 1.1.6
It does not work with jruby 1.2.0RC2
...
Adding new types of properties and relationships can also be done without using
those class methods at an alread existing node (see Neo4j::DynamicAccessorMixin).
...
There are two ways of finding/quering nodes in neo4j - by doing a traversal or by using lucene queries.
...
Neo4j.rb adds the following method for each relationship <relationship_name>?
Example:

  person = Person.new
  person.friends? # => false - no friends yet
  person.friends << person2
  person.friends? # => true - it now has one (or more) friends
...
using ActiveMQ (see the cluster branch).
