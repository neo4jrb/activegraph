...
TODO: the above just goes one level deep in the graph, we should support of any level of traversal
The following example is not implemented yet:

  f.friends{ age > 30}.friends{ name == 'foo'}.each { |n| puts n}

Find all person with more then 5 friends or having the same name

  Person.find{ age > 30 & company == 'foo AB' }

  Company.person{age > 30}

  class Person
    include Neo4j::Node
    relations_from Company, :company
    ...
    def company
      relations.incomming[:company]
    end

    def self.find
        RootNode.company.people.each
    end
  end

parent
  class RootNode
    relations_to Company
  ...
  class Company
     ...
     def self.find
         RootNode.instance.company.each {}
     end
     
     relations_to Person, :people
  end
  ...
  RootNode
  Company.new
  Person.find{ (friends.size > 5) | (name == friends.name) }
