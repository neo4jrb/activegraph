...
You can create an org.neo4j.api.core.Node object by using the Neo4j::Node.new method (!)

  node = Neo4j::Node.new  # => an instance of org.neo4j.api.core.Node

To load a specific node by its ID (see javadoc org.neo4j.api.core.Node.getId()), do
  
  node_id = node.neo_id
  ref_node = Neo4j.load_node(node_id)

The neo_id method works both for the Neo4j::NodeMixin and for org.neo4j.api.core.Node java objects.
