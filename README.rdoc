...
You can create org.neo4j.api.core.Node classes and access their Node ID like

  node = Neo4j::Node.new
  node_id = node._java_node.getId()

To load a specific node by its ID, do
  
  node_id = 0 #reference node
  ref_node = Neo4j.load_node(0)
