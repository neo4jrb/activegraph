...
Example of getting relationships
  ...
  node1.rels.incoming(:friends).nodes.first # => node1  first node of relationship type 'friends'
...
Example:
  ...
  person.rels.outgoing(:best_friends).first.end_node # => other_node (if there is only one relationship of type 'best_friend' on person)
...
You can access the org.neo4j.api.core.EmbeddedNeo class by

  Neo4j.instance

You can create org.neo4j.api.core.Node classes by

  Neo4j::Node.new

You can create a relationship of type org.neo4j.api.core.Relationship by

  a = Neo4j::Node.new
  b = Neo4j::Node.new
  r = a.add_rel(:friends, b)
  r.java_class # => class org.neo4j.impl.core.RelationshipProxy
...
The Neo4j::Node.new method actually returns a java object that implements the org.neo4j.api.core.Node interface.
That java interface is extended with methods so it behavious almost like using the your own Ruby Neo4j::NodeMixin class.
...
==== Neo4j::Node#delete method

Since one can both use the org.neo4j.api.core.Node directly or using the Neo4j::NodeMixin there might be a clash
in method names.
For example the method Neo4j::NodeMixin#del deletes the node and all its relationships.
The org.neo4j.api.core.Node#delete (which is created by Neo4j::Node.new) will raise an exception if not all relationships are already deleted.
...
==== Memberships in lists
 ...
 Example:
   ...
   employee1.list(:employees).next # => employee2
   employee2.list(:employees).next = Neo4j::Node.new
   employee1.list(:employees).size # => 2 # the size counter is available if the :counter parameter is given as shown above
...
If we want to search for exact matches, for example languages codes like 'se', 'it' we must make sure
...
By using the keyword analyzer (instead of the default StandardAnalyzer) we make sure that lucene index everything.
...
Field infos can be used to specify if the
...
If none is specified then the default analyzer - org.apache.lucene.analysis.standard.StandardAnalyzer (:standard) will be used.
