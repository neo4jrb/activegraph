...
== Two Minute Tutorial
  ...
  class Person
    ...
    # define a method 'friends' for relationship friends of type Person
    has :zero_or_more, :friends, Person 

    index 'name'  # adds a lucene index on the name
    index 'age'
...
All neo properties have to be declared (unless you include the DynamicAccessor mixin).
Relations between nodes can be specified by using the relations
...
Example of quering using lucene:

  Person.find (:name => 'kalle') 
  Person.find ('friend.age' => 30) # find people who has a friend of age 30

or DSL, example finding person with name kalle or with age between 20 and 30:

  Person.find ((name == 'kalle') | (age == 20..30)) # => [ person ]
...
TODO: those example are currently broken, relations has been replaced by has and belongs_to.
      ...
      class Customer
        ...
        relations :friends # has default type Neo4j::DynamicRelation 
      end
...
To find a specific relationship use the [] operator:
  ...
  n1.relations.outgoing[n2] = #<Neo4j::Relation:0x134aea2

Or which is better performance wise (since only friends relationships are being traversed):

  n1.relations.outgoing(:friends)[n2] = #<Neo4j::Relation:0x134aea2

=== Deleting a relationship

Use the Neo4j::Relation#delete method.
...
The Neo4j::Node#index method can be used to index relationships to other classes.
...
Now we want to create an index so that we can find all orders a customer has made.

  Order.index(Customer, 'Customer.name'){name}

(That means, when a Customer node changes then eval the block ({name}) in that node context and
put it in the index with the key 'Customer.name')
...
Let's create a customer and one order for that customer
    ...
    Neo4j::Transaction.run do
      ...
      # orders method is generated by 'contains :zero_or_more, Order' from above
      cust.orders << order 
    end

We can now find all order a customer have made:
