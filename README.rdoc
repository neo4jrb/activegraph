...
Make sure that you have installed RSpec and Rake, otherwise type:

  gem install rspec  # or jruby -S gem install rspec
  gem install rake   # or jruby -S gem install rake
...
Adding new types of properties and relationships can also be done without using those class methods at an alread existing node.
...
If a node is deleted then all its (incoming) relationship will also be deleted
...
The has_one and has_many create a convenient method for traversals and managing relationships to other nodes.
Example:
...
the same time - Neo4j::NodeMixin#traverse.

=== Example on Relationships

A more complex example that shows how to navigate from incoming and
outgoing relationships:

      class Role
        include Neo4j::RelationMixin # Relationships is also Ruby objects
        property :name
      end
...
Neo4j.rb is used to map Ruby objects to nodes and relationships in a network.
It supports two ways of retrieval/quering - by Neo4j traversal or
by using Lucene indexes.
...
Relationships has to be declared by using the 'has_n' or 'has_one'
...
The Neo4j::NodeMixin#relations method can be used to find incoming or outgoing relationship objects.
Example of listing all types of outgoing (default) relation objects (of depth one) from the me node.
