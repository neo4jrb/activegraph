...
Example of creating a relationship
  ...
  node1.add_relationship(node2, :friends)

Example of getting relationships

  node1.relationships.include?(node2) # => true - it implements enumerable and other methods
  ...
  node1.relationships(:friends).first # => the first relationship of type :friends
...
declaring those propertie/relationships (like the example above), see Neo4j::DynamicAccessorMixin,  or the [] and add_relationship methods.
