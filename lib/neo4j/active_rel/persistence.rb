module Neo4j
  module ActiveRel
    module Persistence
      ...
      def exist?
        _persisted_rel && _persisted_rel.exist?
      end
      ...
      def destroy
        _persisted_rel && _persisted_rel.del
        @_deleted = true
      end
      ...
      def update_model
        if @changed_attributes && !@changed_attributes.empty?
          ...
          _persisted_rel.update_props(changed_props)
      ...
      def _create_rel(*args)
        ...
        session.create_rel(rel_type, start_node, end_node, props)
      end
