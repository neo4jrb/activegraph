module ActiveGraph::ActiveNode
  module HasN
    ...
    class AssociationProxy
      ...
      # Default
      def inspect
        formatted_nodes = ::ActiveGraph::ActiveNode::NodeListFormatter.new(result_nodes)
      ...
      def result_ids
        result.map do |object|
          object.is_a?(ActiveGraph::ActiveNode) ? object.id : object
        end
      end
      ...
      def method_missing(method_name, *args, &block)
        ...
        cache_query_proxy_result if !cached? && !target.is_a?(ActiveGraph::ActiveNode::Query::QueryProxy)
        clear_cache_result if target.is_a?(ActiveGraph::ActiveNode::Query::QueryProxy)
      ...
      def map_results_as_nodes(result)
        result.map do |object|
          object.is_a?(ActiveGraph::ActiveNode) ? object : @query_proxy.model.find(object)
        end
      end
    ...
    module ClassMethods
      ...
      def association_query_proxy(name, options = {})
        ...
        ActiveGraph::ActiveNode::Query::QueryProxy.new(association_target_class(name),
                                                 associations[name],
                                                 {driver: neo4j_driver,
                                                  query_proxy: query_proxy,
      ...
      def default_association_query_proxy
        ActiveGraph::ActiveNode::Query::QueryProxy.new("::#{self.name}".constantize, nil,
      ...
      def build_association(macro, direction, name, options)
        ...
        ActiveGraph::ActiveNode::HasN::Association.new(macro, direction, name, options).tap do |association|
