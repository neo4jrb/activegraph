  ...
  module Query
    ...
    module ClassMethods
      ...
      def find_by_id(key, driver = nil)
        ...
        query ||= ActiveGraph::ActiveBase.new_query(options)
      ...
      def deprecation_warning!
        ActiveSupport::Deprecation.warn 'The ActiveGraph::ActiveRel::Query module has been deprecated and will be removed in a future version of the gem.', caller
      end

      def where_query
        deprecation_warning!
        ActiveGraph::ActiveBase.new_query.match("#{cypher_string(:outbound)}-[r1:`#{self._type}`]->#{cypher_string(:inbound)}")
      end

      def all_query
        deprecation_warning!
        ActiveGraph::ActiveBase.new_query.match("#{cypher_string}-[r1:`#{self._type}`]->#{cypher_string(:inbound)}")
      end
