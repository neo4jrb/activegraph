...
describe "Neo4j::Rails::Model relationships", :type => :integration do
  ...
  let(:node_with_relationship) do
    ...
    #  end
    #end
    rel_class = relationship_with_no_property_class
    create_model do
      has_one(:other_node)
      has_one(:foobar).relationship(rel_class)
      has_n(:baaz)
  ...
  describe "Neo4j::Rails::Model Relationships", :type => :integration do
    describe "incoming" do
      it "created incoming relationship can be accessed from outgoing node" do
        ...
        a.save
        # then
        a.incoming(:foo).count.should == 1
        a.incoming(:foo).should include(b)

        b.outgoing(:foo).count.should == 1
        ...
        context "when all relationships are persisted" do
          ...
          describe "to_other" do
            it "returns an Enumerable of all found relationships between the given nodes" do
              @a.rels.to_other(@d).to_a.count.should == 2
              rels = @a.rels(:outgoing, :bar).to_other(@d).map { |n| n[:name] }
              rels.should =~ ['ad1', 'ad2']
            end
      ...
      context "find a specific relationship type (#rels(:rel_type))" do
        context "when all relationships are persisted" do
          it "returns all relationships of the given type" do
            @a.rels(:outgoing, :foo).count.should == 2
            @a.rels(:outgoing, :bar).map { |x| x[:name] }.compact.should =~ %w[ad1 ad2]
          end

          describe "delete all found relationship of a given type" do
            it "deletes with #delete_all" do
              @a.rels(:outgoing, :foo).count.should > 0
              @a.rels(:outgoing, :foo).delete_all
              @a.rels(:outgoing, :foo).count.should == 0
            end

            it "deletes with #destroy_all" do
              @a.rels(:outgoing, :foo).count.should > 0
              @a.rels(:outgoing, :foo).destroy_all
              @a.rels(:outgoing, :foo).count.should == 0
            end
          end
          ...
          describe "to_other" do
            it "return an Enumerable of all found relationships between the given nodes" do
              @a.rels(:outgoing, :foo).to_other(@b).count.should == 1
              rels = @a.rels(:outgoing, :bar).to_other(@d).map { |n| n[:name] }
              rels.should =~ ['ad1', 'ad2']
            end
        ...
        context "when no relationships are persisted" do
          it "returns all relationships if no type is given" do
            @a1.rels(:outgoing, :foo).count.should == 2
            @a1.rels(:outgoing, :bar).map { |x| x[:name] }.compact.should =~ %w[ad11 ad21]
          end

          describe "delete all relationship of given type" do
            it "delete with #del deletes all relationships" do
              @a1.rels(:outgoing, :foo).count.should > 0
              @a1.rels(:outgoing, :foo).delete_all
              @a1.rels(:outgoing, :foo).count.should == 0
              @a1.rels.count.should > 0
            end
            ...
            it "delete with #destroy deletes all relationships" do
              @a1.rels(:outgoing, :foo).count.should > 0
              @a1.rels(:outgoing, :foo).destroy_all
              @a1.rels(:outgoing, :foo).count.should == 0
              @a1.rels.count.should > 0
            end
          end
          ...
          describe "to_other" do
            it "raise an exception since it only allows finding persisted relationships" do
              lambda { @a1.rels(:outgoing, :foo).to_other(@b1).to_a }.should raise_error
            end
          end
    ...
    describe "has_n from" do
      ...
      it "should find the node both ways" do
        @a.known_by.count.should == 1
        @a.known_by.should include(@b)
        @b.friends.count.should == 1
      ...
      it "can be found using incoming and outgoing" do
        @b.outgoing(:friends).count.should == 1
        @b.outgoing(:friends).should include(@a)
        @a.incoming(:friends).count.should == 1
    ...
    describe "has_one, has_n, outgoing" do
      ...
      it "should find the relationship using #outgoing method" do
        ...
        a.outgoing(:friends).count.should == 2
      ...
      it "should find the relationship using #has_n friends_rels method when created with outgoing method" do
        ...
        a.friends_rels.to_a.count.should == 2
        a.save!
        a.friends_rels.to_a.count.should == 2
      end
      ...
      it "should only find both persisted and none persisted relationship before saving it" do
        ...
        d = clazz.new
        a.friends << d
        a.friends_rels.to_a.count.should == 3
        a.friends.to_a.count.should == 3
      ...
      it "#del deletes the relationship without needing to call save" do
        ...
        rel = a.rels(:outgoing, :friends).find(b)
      ...
      describe "nested nodes two level deep" do
        ...
        it "deleting relationships only when save is called" do
          ...
          jack.knows_rels.to_a.count.should == 2
  ...
  describe "RelationshipWithNoProperty", :type => :integration do
    ...
    subject do
      relationship_with_no_property_class.new(:foo, @start_node, @end_node)
    end

    it "should persist" do
      subject.save
      relationship_with_no_property_class._all.count.should == 1
      subject.class.all.to_a.count.should == 1
    end
    ...
    context "when there's lots of them" do
      ...
      it "should be possible to #destroy_all" do
        subject.class.all.to_a.count.should == 4
  ...
  describe "SettingRelationship", :type => :integration do
    ...
    context "create a Neo4j::Rails::Relationship" do
      ...
      it "add an incoming should set the outgoing" do
        ...
        @end_node.outgoing(:other_node).to_a.count.should == 1
        @start_node.incoming(:other_node).to_a.count.should == 1
      ...
      it "adding many different relationships" do
        ...
        c.incoming(:foo).count.should == 1
        c.incoming(:foo).should include(@start_node)
        b.incoming(:other_node).count.should == 1
        b.incoming(:other_node).should include(@start_node)
        a.incoming(:other_node).count.should == 1
      ...
      it "should find the relationship using #incoming method" do
        ...
        a.outgoing(:qwe) << b
        a.outgoing(:qwe) << c
        a.outgoing(:qwe).count.should == 2
        b.incoming(:qwe).count.should == 1
        b.outgoing(:qwe).count.should == 0
        c.incoming(:qwe).count.should == 1
        c.outgoing(:qwe).count.should == 0
        b.incoming(:qwe).should include(a)
        c.incoming(:qwe).should include(a)
        a.save
        a.outgoing(:qwe).count.should == 2
        b._java_node.incoming(:qwe).count.should == 1
        b.incoming(:qwe).count.should == 1
        b.outgoing(:qwe).count.should == 0
        c.incoming(:qwe).count.should == 1
        c.outgoing(:qwe).count.should == 0
    ...
    context "setting RelationshipWithNoProperty" do
      ...
      it { should be_kind_of(relationship_with_no_property_class) }

      it "should create the correct relationship class after save" do
        @start_node.foobar_rel.should be_kind_of(relationship_with_no_property_class)
        @start_node.save
        @start_node.foobar_rel.should be_kind_of(relationship_with_no_property_class)
      end
      ...
      it "should have no incoming" do
        @start_node.rels(:incoming, :foobar).should be_empty
        @start_node.rels(:outgoing, :foobar).count.should == 1
      end
    end
      ...
      context "when validates associated" do
        context "when a nested model is invalid" do
          it "should make parent model invalid" do
            ...
            member.descriptions.count.should == 1
          ...
          it "should allow delete_all" do
            ...
            member = Member.create.tap do |member|
              member.descriptions << description
              member.save
              member.descriptions_rels.delete_all
              member.descriptions << description
              member.save
            end
